...
class Error { // 잘못된 입력을 분석해서 에러메세지를 출력하는 class
...
class todoProgram {
  ...
  addError(newTaskName) { // 같은 task가 존재하면 true 않으면 false 반환하는 method
  ...
  // todo.update("2$doing");
  // [error] 2번은 이미 doing입니다.

  // > todo.update("38$ doNe ");
  // [error] 38번 아이디는 존재하지 않습니다.

  // > todo.update("3$doing");
  // [error] done 상태에서 doing상태로 갈 수 없습니다. 
  updateError(newId, newStatus) {
    for (let task of this.taskList) {
  ...
  removeError(newId) { // 같은 id가 존재하면 false 않으면 true 반환하는 method
    for (let task of this.taskList) {
      const { id } = task;
  ...
  update(nowStatus) { // 현재상태를 갱신 하는 함수
    if (typeof nowStatus === 'string') {
      nowStatus = nowStatus.split('$');
      nowStatus = {
        id: parseInt(nowStatus[0], 10),
        nextstatus: nowStatus[1]
      }
    }
    let newStatus = nowStatus.nextstatus.toLowerCase();
    let oldStatus;
    let taskName;
    if (this.updateError(nowStatus.id, nowStatus.nextstatus)) return this.newLine();
